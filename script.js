...
function startRandomQuest (questsObj, userLevel, headers) {
    ...
    if (questsArr.length > 0) {
        document.getElementById('randomQuestButton').addEventListener('click', async () => {
            ...
            await fetch("https://habitica.com/api/v3/groups/party/quests/invite/" + randomQuest, {method: "post", headers}).then(resp => resp.json);
...
document.getElementById('submit-api-key').addEventListener("click", async () => {
    ...
    } = await fetch('https://habitica.com/api/v3/user', {method: 'GET', headers}).then(r => r.json());
    ...
    } = await fetch('https://habitica.com/api/v3/user/inventory/buy', {method: 'GET', headers}).then(r => r.json());
    
    const {
        data : {
            quest
        }
    } = await fetch('https://habitica.com/api/v3/groups/party', {method: 'GET', headers})
                        .then(r => r.json())
